...
| Inspector (§6) | mixed-value multi-selection editing (relative scrub, per-node math, one undo per gesture), per-section subscriptions + memo + per-node revision, sticky selection header, Pinned tab (`__pinnedProps`), `=` expression toggle with autocomplete/error/pick-whip, keyframe lane per row, section presets, colour-management readout, modifier-stack chips, `AppearanceSection`/`TrackMotionSection` split | **Effects sub-tab — NOT shipped** (corrected 2026-09-15): Properties has no Effects sub-tab; applied effects live in the separate Effect Controls panel and the library in the Effects panel |
...
| Sidebars (2026-09-15) | Rails cut from 7 + 14 unlabelled icons to **Project, Library, AI** (left) and **Properties, Audio** (right); every other panel on demand via Window ▸ Panels and a per-rail "+"; rail tabs labelled; Layers' comp list moved into Project (and now lists the open pristine comp); Info & Audio folded into Audio; one-time layout migration (`LAYOUT_SCHEMA_VERSION` 2) | Effects / Presets / Plugins as Library sections and Text as a Properties section are a separate pass |
