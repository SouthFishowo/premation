 ...
 *
 * `cornerRadii` (TL → TR → BR → BL, already clamped — see `clampCornerRadii`)
 * rounds the rect's corners INTO the outline as flattened quarter arcs. This is
 * how a rounded rect keeps its corners under the operator chain: the chain
 * converts the primitive to an explicit path, and the rasterizer's
 * `cornerRadii` honouring lives in the rect branch it no longer takes — so
 * without this, adding ANY operator squared the corners off as a side effect.
 * Absent or all-zero radii emit the four sharp corners byte-identically to
...
export function shapeOutline(
  ...
  if (cornerRadii && cornerRadii.some((r) => r > 0.25)) {
    return roundedRectOutline(w / 2, h / 2, cornerRadii, subdivide);
  }
  const corners: Pt[] = [
    { x: -w / 2, y: -h / 2 },
    { x: w / 2, y: -h / 2 },
...
/** Rounded-rect outline: four quarter arcs (flattened, adaptive density)
 *  joined by straight edges, TL → TR → BR → BL, centred at (0, 0). */
function roundedRectOutline(
  ...
  // Arc centres and start angles, walking the same direction the sharp-corner
  // outline does (TL → TR → BR → BL). With r = 0 the centre IS the corner, so
  // a sharp corner falls out of the same expression as a single point.
  const corners = [
    { cx: -hw + radii[0], cy: -hh + radii[0], a0: Math.PI, r: radii[0] },
    { cx: hw - radii[1], cy: -hh + radii[1], a0: Math.PI * 1.5, r: radii[1] },
    { cx: hw - radii[2], cy: hh - radii[2], a0: 0, r: radii[2] },
    { cx: -hw + radii[3], cy: hh - radii[3], a0: Math.PI * 0.5, r: radii[3] },
  ] as const;
  const arcs: Pt[][] = corners.map(({ cx, cy, a0, r }) => {
    if (r <= 0.25) return [{ x: cx, y: cy }];
    const steps = Math.max(2, Math.min(96, Math.ceil((r * Math.PI * 0.5) / CORNER_ARC_MAX_CHORD_PX)));
    const pts: Pt[] = [];
  ...
  for (let k = 0; k < 4; k++) {
    const arc = arcs[k]!;
    out.push(...arc);
    // The straight edge to the next corner's arc, with the same `subdivide`
    // interior points the sharp outline inserts (pucker/twist density).
    const a = arc[arc.length - 1]!;
    const b = arcs[(k + 1) % 4]![0]!;
    if (subdivide > 0 && Math.hypot(b.x - a.x, b.y - a.y) > 1e-6) {
